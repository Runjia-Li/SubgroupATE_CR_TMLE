...
## Skip next 10 lines if required packages are already installed
# install.packages("devtools")
# withr::with_envvar(c(R_REMOTES_NO_ERRORS_FROM_WARNINGS="true"), 
#                    devtools::install_github("bozenne/BuyseTest", ref = "competingRisks"))
...
#### The working directory should be the "code" folder.
...
true_cate<-function(tp, data.test, beta1, beta2, omega1, omega2, gamma, p){
  ...
  # imp.Z.bin<-which(beta1[1:5]!=0|omega1[1:5]!=0)
...
TMLE_FG<-function(data.train, data.test, tp=tp, Q=2,learner,Z_ind=NULL, Inter_ind=NULL, 
                  treat_Z_ind=NULL,censor_Z_ind=NULL,split=FALSE, SL=FALSE,LS=FALSE,ITE=FALSE){
  ...
  for (l in 1: length(train.lst)){
    ...
    if (learner=="T"){
      ...
      if (LS==FALSE){ 
        ...
        # names(weight.trt)[1]<-names(weight.trt)[2]<-names(weight.cnt)[1]<-names(weight.cnt)[2]<-"A"          
    ...
    # truth.train<-true_cate(tp=tp, data.train, beta1, beta2, omega1, omega2, gamma, p)
    ...
    if (length(Z_ind)==length(Inter_ind)){
      ...
      if (SL==TRUE){
        ...
        # Z<-as.data.frame(Z.train[,2])
        colnames(Z)<-paste0("Z.",treat_ind[!treat_ind%in% Inter_ind])
        # treat.SL.library <- c('SL.glm','SL.gam','SL.polymars','SL.glmnet')#  'SL.glmnet', 'SL.gbm',    #'SL.earth','SL.ranger',
        ...
        # summary(g_w.glm-g_true)
        g_w<-g_w.glm
      }
    }
    H_1<--1/(g_w); H_0<-1/(1-g_w)
    H_A<-ifelse(A.train==1,H_1, H_0)
    # g_true=1 / (1 + exp(-(Z.train%*%gamma)))
    ...
    if (SL==FALSE){
      ...
      # bhaz.cox[, surv.C:=exp(-chaz0.1)]
      # bhaz.cox<-cbind(bhaz.cox,surv.C)
      # colnames(bhaz.cox)[6:ncol(bhaz.cox)]<-paste0("G_",c(1:nrow(data.train)))
    }else{
     ...
     #  censor.mat<-matrix(rep(lambda0*exp(Z.train%*%matrix(censor_beta)),length(unique.times)),byrow=TRUE,nrow=length(unique.times))
     # G.true<-censor.mat*exp(-censor.mat*matrix(rep(unique.times,nrow(data.train)),byrow=FALSE,ncol=nrow(data.train)))
    }
    ########### Step 2.1: Update the initial estimate of E(Y|A,Z) ###############
    # calculate clever covariate h_a
    # the parts that are not updated by fitting submodel
    # start_time<- Sys.time()
    # start_time<- Sys.time()
    ...
    # end_time1 <- Sys.time()
    # (arraytime1<- start_time-end_time1) # 0.09s
    
    # summary((surv.C-matrix(rep((unique.times-max(data.train$event.time[which(data.train$event.type==0)]))/(min(unique.times)-max(data.train$event.time[which(data.train$event.type==0)])),nrow(data.train)),byrow=FALSE,ncol=nrow(data.train)))[1:604506])
    
    ############################
    # initial haza and F
    # haza is the matrix of hazard (\lambda_t,id=\lambda_0,t,id * exp(X_id*\beta))
    # le.t.obs<-do.call("rbind",lapply(unique.times,function(tt) (tt<=data.train$event.time)))
    # w.Y0<-do.call("rbind",lapply(unique.times,function(tt) {
    #   tt.ind=which(unique.times==tt)
    #   lapply(data.train$event.time,function(tobs) {
    #           tobs.ind=which(data.train$event.time==tobs)
    #           (tobs>=tt)+
    #            (tobs<tt & data.train$event.type[tobs.ind]==2)*surv.C[tt.ind,tobs.ind]/surv.C[tobs.ind,tobs.ind]
    #            })}))
    ...
    for (j in 1:length(tp)){
      ...
      while (abs(eps.hat)>=0.001){
        ...
        # h1[F1==1]=ccpart1_1[[j]][F1==1];h0[F0==1]=ccpart1_0[[j]][F0==1]; #no big difference
        h1[F1>0.99]=ccpart1_1[[j]][F1>0.99];h0[F0>0.99]=ccpart1_0[[j]][F0>0.99]; #no big difference
        # ha<-ccpart1[[j]]*(matrix(rep(1-Fa[tk,],nrow(Fa)),nrow=nrow(Fa),byrow=TRUE)/(1-Fa)) 
        ...
        F1<-apply(haz1, 2,function(x) 1-exp(-cumsum(x))); #xxx<-apply(F1,2,function(xx) na.locf(xx)) #bhaz.cox[, hazard0:=na.locf(hazard)]
  ...
  # CATE.test<-merge(trueITE, CATE.train.matrix, by=paste0(rep("Z.",length(imp.Z.bin)),imp.Z.bin))  
  # diff.ITE.test<-CATE.test[,colnames(CATE.test) %in% paste0(rep("DF_",length(tp)),c(1:length(tp)))]-
  #   CATE.test[,colnames(CATE.test) %in% colnames(trueITE)[!colnames(trueITE)%in% paste0(rep("Z.",length(imp.Z.bin)),imp.Z.bin)]]
  # if (length(tp)>1){
  #   bias.ITE.test<-apply(abs(diff.ITE.test),2,mean)
  #   rmse.ITE.test<-apply(diff.ITE.test,2,function(x) sqrt(mean(x^2)))
  # }else{
  #   bias.ITE.test<-mean(abs(diff.ITE.test))
  #   rmse.ITE.test<-sqrt(mean((diff.ITE.test^2)))
  # } 
  if (ITE==TRUE){
...
FG_S<-function(data.train, data.test, tp=tp,Z_ind=NULL, Inter_ind=NULL,LS=FALSE,SL=FALSE,ITE=FALSE){ # ? time points
  ...
  if (LS==FALSE){ 
    ...
    lam.path <- 10^seq(log10(0.01), log10(0.000001), length = 100)
    # weight <- 1/abs(crr(data.train$event.time, data.train$event.type, cbind(A.train,Z.train[,Z_ind],Inter.train[,Inter_ind]))$coef)
...
FG_T<-function(data.train, data.test, tp=tp,Z_ind=NULL,Inter_ind=NULL,LS=FALSE,SL=FALSE,ITE=FALSE){ # ? time points
  # baseline<-data.train%>%filter(Z.1==0&Z.2==0&Z.3==0&Z.4==0&Z.5==0)
  # phat<-sum(baseline$event.type==1)/(sum(baseline$event.type==1)+sum(baseline$event.type==2))
  ...
  if (LS==FALSE){ 
    ...
    # names(weight.trt)<-names(weight.cnt)<-"A"    
  ...
  colnames(CATE.t.matrix)<-c(paste0(rep("Z.",length(imp.Z.bin)),imp.Z.bin),
                             paste0(rep("DF_",length(tp)),c(1:length(tp))),
                             paste0(rep("SE_",length(tp)),c(1:length(tp))))
  # diff.ITE.test<-CATE.t.lst[,colnames(CATE.t.lst) %in% paste0(rep("DF_",length(tp)),c(1:length(tp)))]-
  #   trueITE[,!colnames(trueITE)%in% paste0(rep("Z.",length(imp.Z.bin)),imp.Z.bin)]
  # if (length(tp)>1){
  #   bias.ITE.test<-apply(abs(diff.ITE.test),2,mean)
  #   rmse.ITE.test<-apply(diff.ITE.test,2,function(x) sqrt(mean(x^2)))
  # }else{
